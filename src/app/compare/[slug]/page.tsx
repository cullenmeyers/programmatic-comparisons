...
import { loadPageBySlug, listPageSlugs, slugifyCompare, type PageDoc } from "@/lib/pages";
...
function FAQJsonLd({ faqs }: { faqs: Array<{ q: string; a: string }> }) {
  // If no FAQs, avoid outputting empty schema
          ...
          <ul className="space-y-3 list-disc pl-5">
            {section.bullets.map((b, i) => (
              <li key={i}>
                ...
                <div className="text-sm leading-6 opacity-80">{b.why_it_matters}</div>
              </li>
            ))}
          </ul>
        </section>
          ...
          <div className="space-y-4">
            {section.items.map((it, i) => (
              <div key={i} className="rounded-lg border p-4">
                ...
                <div className="text-sm leading-6 opacity-85">{it.fails_when}</div>
                <div className="mt-3 font-medium">What to do instead</div>
                <div className="text-sm leading-6 opacity-85">{it.what_to_do_instead}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="space-y-2">
          ...
          <ul className="space-y-2 list-disc pl-5">
            {section.rules.map((r, i) => (
              <li key={i} className="leading-7">{r}</li>
            ))}
          </ul>
        </section>
      );
      ...
      <header className="space-y-3">
        ...
        <div className="rounded-xl border p-5 space-y-2">
          ...
          <p className="text-sm leading-6 opacity-80">
            <span className="font-medium">Rule:</span> {doc.verdict.decision_rule}
          </p>
        </div>
      </header>
        ...
        <ul className="list-disc pl-5 space-y-1">
          {items.map(({ rp, relatedSlug, exists }, i) => (
            <li key={i} className="text-sm leading-6">
              {exists ? (
                <Link className="underline underline-offset-4" href={`/compare/${relatedSlug}`}>
