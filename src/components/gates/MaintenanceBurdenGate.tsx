...
export default function MaintenanceBurdenGate({
  xName,
  yName,
  variant = "embed",
  disabled = false,
  ...
  const output = useMemo(() => {
    if (choice === "X") {
      return [
        `${xName} exceeds your maintenance tolerance.`,
        `Eliminate ${xName} and continue evaluation with ${yName}.`,
      ];
    }
    if (choice === "Y") {
      return [
        `${yName} exceeds your maintenance tolerance.`,
        `Eliminate ${yName} and continue evaluation with ${xName}.`,
      ];
    }
    return [
      `Neither tool exceeds your maintenance tolerance.`,
  ...
  const effectiveOutput =
    disabled && lockedMessage ? [lockedMessage, ""] : output;
    ...
    <section className={containerClass}>
      <div className="space-y-1">
        ...
        <h2 className="text-xl font-semibold tracking-tight">
          Maintenance Burden Gate
        </h2>
        <p className="text-sm leading-6 text-black/65">
          Pick the option that will require too much ongoing upkeep to keep
          working well.
        </p>
      </div>
      ...
      <div className={questionClass}>
        <div className="text-sm font-medium text-black/75">
          Which tool will require too much ongoing maintenance?
        </div>
        ...
        <div className="flex flex-col gap-2">
          <label className="flex items-center gap-2 text-sm">
            <input
              type="radio"
              name={`mbg-${xName}-${yName}`}
              value="X"
              checked={choice === "X"}
              onChange={() => setChoice("X")}
              disabled={disabled}
            />
            <span className="text-black/80">{xName} will require too much</span>
          </label>
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              ...
              disabled={disabled}
            />
            <span className="text-black/80">{yName} will require too much</span>
          </label>
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              ...
              disabled={disabled}
            />
            <span className="text-black/80">Neither will require much</span>
          </label>
        </div>
